...
const INTEGRATIONS: IntegrationConfig[] = [{
  ...
  fields: [{
    ...
    helpText: 'Your Razorpay API Key ID'
  }, {
    key: 'keySecret',
    label: 'Key Secret',
    type: 'password',
    ...
    helpText: 'For connected accounts only'
  }]
}, {
  id: 'biotime',
  ...
  fields: [{
    ...
    label: 'API URL',
    type: 'url',
    placeholder: 'https://your-biotime-server.com/api',
    helpText: 'Your BioTime server API endpoint'
  }, {
    key: 'apiKey',
    label: 'API Key',
    type: 'password',
  ...
  fields: [{
    ...
    helpText: 'Generate from Zoho API Console'
  }]
}, {
  id: 'slack',
  name: 'Slack',
  ...
  fields: [{
    ...
    helpText: 'Create an incoming webhook in Slack'
  }, {
    key: 'channel',
    label: 'Default Channel',
    type: 'text',
    placeholder: '#roster-updates'
  }]
}, {
  id: 'google-calendar',
  ...
  fields: [{
    ...
    type: 'text',
    placeholder: 'primary',
    helpText: 'Use "primary" for the main calendar'
  }]
}, {
  id: 'zapier',
  name: 'Zapier',
  ...
  fields: [{
    ...
    helpText: 'Create a Zap with Webhook trigger'
  }]
}];
export default function SettingsPage() {
  ...
  return <div className="flex flex-col h-full">
      <DashboardHeader title="Settings" subtitle="Configure roster rules and preferences" />
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6 max-w-4xl">
        ...
        <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
          ...
          <div className="p-6">
            <div className="flex items-center justify-between">
              ...
              {loading ? <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /> : <Select value={status} onValueChange={value => handleStatusChange(value as UserStatus)} disabled={saving}>
                  <SelectTrigger className="w-40">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(['available', 'on-leave', 'unavailable'] as UserStatus[]).map(s => <SelectItem key={s} value={s}>
        ...
        <div className="bg-card rounded-xl border border-border/50 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            {INTEGRATIONS.map(integration => <IntegrationCard key={integration.id} config={integration} isConnected={connectedIntegrations.has(integration.id)} onConnect={handleConnect} onDisconnect={handleDisconnect} />)}
          </div>
        </div>
